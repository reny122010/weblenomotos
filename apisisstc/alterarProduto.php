...
else
{
 ...
 if ( $stmt = $conn->prepare("UPDATE tbproduto SET nome=?,preco=?,custo = ?,quantidade = ?,unidade = ? WHERE codigodebarras = ?")) 
 {
    $stmt->bind_param("sddisi",$Produto->nome,$Produto->preco,$Produto->custo,$Produto->quantidade,$Produto->unidade,$Produto->codigodebarras);
    ...
    if (!empty($stmt->error)) 
    {
        $arr = array('retorno' => 1,'menssagem' =>'Erro ao alterar o produto.', 'erro'=> $stmt->error);
        echo json_encode($arr);
        die();
    }
    else
    {
        $arr = array('retorno' => 0, 'menssagem' =>'Produto alterado com sucesso.');
        echo json_encode($arr);     
...
{
    $arr = array('retorno' => 1,'menssagem' =>'Erro ao alterar o produto.','erro'=>'not $conn->prepare');
    echo json_encode($arr); 
    die();
}
