...
if(isset($_GET['telefone']) or !empty($_GET['telefone']))
{
...
else
{
	...
	if (!($result->num_rows === 0)) {
    	$arr = array('retorno' => 1,'menssagem' =>'Cliente já cadastrado.');
		echo json_encode($arr);
		die();
	}

 if ( $stmt = $conn->prepare("INSERT INTO tbcliente (bairro,cidade,cpf,limite,logradouro,nome,sobrenome,telefone) VALUES (?,?,?,?,?,?,?,?)")) 
 {
 	$stmt->bind_param("ssiissss",$Cliente->bairro,$Cliente->cidade,$Cliente->cpf,$Cliente->limite,$Cliente->logradouro,$Cliente->nome,$Cliente->sobrenome,$Cliente->telefone);
	...
	if (!empty($stmt->error)) 
	{
	 	$arr = array('retorno' => 1,'menssagem' =>'Erro ao inserir o cliente.', 'erro'=> $stmt->error);
		echo json_encode($arr);
		die();
	}
	else
	{
		$arr = array('retorno' => 0, 'menssagem' =>'Cliente adicionado com sucesso.');
		echo json_encode($arr);		
...
{
	$arr = array('retorno' => 1,'menssagem' =>'Erro ao inserir o cliente.','erro'=>'not $conn->prepare');
	echo json_encode($arr);	
	die();
}
